...
- 交互模式向计划生成器提供最近 5 轮紧凑任务状态，包含工作目录、命令、最终状态和是否实际执行。
- 取消、预览、阻止和预检失败会标记为未执行，不能被模型当作文件系统事实；退出程序后上下文清空。
- 单次任务与批处理不共享会话上下文，本地文件系统检查始终优先于上下文。
...
- 删除候选必须完整输入 `yes`，不能用回车或 `y` 默认接受；选定后仍会重新进行预检、风险分析和删除确认。
- `cp` 或 `mv` 缺少目标时要求补充目标。
- 单文件复制到自身时，可直接回车采用类似 `README_copy.md` 的不冲突名称。
- 修正命令后重新检查；仍然不合法时不会执行。
- 路径会规范化后再判断是否位于当前工作区，降低 `..` 和符号链接逃逸风险。
- 当前工作区根目录及其父目录是永久保护目标，任何运行方式都不能删除。
- “删除”“清理”等没有具体对象的输入必须先澄清，不能默认解释为当前项目。
...
- 如果当前目录被外部程序删除，交互模式会回退到最近存在的父目录，必要时回到用户主目录。
...
- 删除当前工作区本身或包含它的父目录始终阻止，即使风险正则只判为 WARN。
